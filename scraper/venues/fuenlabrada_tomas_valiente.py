...
VENUE_NAME = "Teatro Tomás y Valiente"
VENUE_KEYWORDS = ["tomás y valiente", "tomas y valiente"]
...
async def scrape_event_detail(page, url):
    ...
    try:
        ...
        company = ""
        if artistas_raw:
            # Multiple artists may be separated by whitespace
...
async def scrape_tomas_y_valiente():
    """Main scraper function for Teatro Tomás y Valiente (Fuenlabrada)."""
...
if __name__ == "__main__":
    res = asyncio.run(scrape_tomas_y_valiente())
    for e in res:
